...
def ProcessingData(DataDirectoryPath, data_fname, tmp_fname):
    ...
    with open(data_fname, 'r') as f:
        for line in f:
            ...
            if feat_type in ['ServiceList', 'BroadcastReceiverList', 'IntentFilterList', 'ActivityList']:
                continue
    ...
    for year in range(2012, 2019):
        ...
        for midx, m in enumerate(month_list):
            if midx < 9:
                ...
                month = str(m)
            MalDir = '/space1/mldroid_drebin_reduced/malware/%s/%s' % (year, month)
            GoodDir = '/space1/mldroid_drebin_reduced/benign/%s/%s' % (year, month)
